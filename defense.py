...
class Scope(Defense):
    ...
    def exec(self, client_models, global_model, num_dps, g_user_indices, device, attack_round=False, *args, **kwargs):

        vectorize_nets = [vectorize_net(cm).detach().cpu().numpy() for cm in client_models]
        vectorize_global_model = vectorize_net(global_model).detach().cpu().numpy()

        pre_metric_dis = [0.0] * len(vectorize_nets)

        for i, g_i in enumerate(vectorize_nets):
            pre_metric = np.power(abs(g_i - vectorize_global_model) / (abs(g_i) + abs(vectorize_global_model)),
                                  2) * np.sign(g_i - vectorize_global_model)
            pre_metric_dis[i] = pre_metric

        sum_dis = [0.0] * len(vectorize_nets)
        cos_dis = np.zeros((len(vectorize_nets), len(vectorize_nets)))
        for i, g_i in enumerate(pre_metric_dis):
            for j in range(i, len(pre_metric_dis)):
                g_j = pre_metric_dis[j]
                cos_dis[i, j] = float(
                    (1 - (np.dot(g_i, g_j) / (np.linalg.norm(g_i) * np.linalg.norm(
                        g_j)))))  # Compute the different value of cosine distance
                if abs(cos_dis[i, j]) < 0.000001:
                    cos_dis[i, j] = 100
                cos_dis[j, i] = cos_dis[i, j]
                sum_dis[i] += cos_dis[i, j]
                sum_dis[j] += cos_dis[i, j]

        choice = np.argmin(sum_dis)
        cluster = [choice]
        for i in range(len(vectorize_nets)):
            tmp = np.argmin(cos_dis[choice])
        ...
        selected_num_dps = np.array(num_dps)[cluster]
        reconstructed_freq = [snd / sum(selected_num_dps) for snd in selected_num_dps]

        logger.info("Num data points: {}".format(num_dps))
        logger.info("Num selected data points: {}".format(selected_num_dps))
        logger.info("The chosen ones are users: {}, which are global users: {}".format(cluster,
                                                                                       [g_user_indices[ti] for ti in
                                                                                        cluster]))

        aggregated_grad = np.average(np.array(vectorize_nets)[cluster, :], weights=reconstructed_freq,
                                     axis=0).astype(np.float32)
