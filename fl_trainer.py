...
def get_results_filename(poison_type, attack_method, model_replacement, project_frequency, defense_method, norm_bound,
    ...
    elif defense_method in ("multi-metrics", "scope", "krum", "multi-krum", "rfa"):
...
def train(model, device, train_loader, optimizer, epoch, log_interval, criterion, pgd_attack=False, eps=5e-4,
          model_original=None,
          proj="l_2", project_frequency=1, adv_optimizer=None, prox_attack=False, wg_hat=None, attack_method=None):
    """
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if pgd_attack and attack_method == "constrained_and_scale":
            wg_hat_vec = parameters_to_vector(list(wg_hat.parameters()))
            model_vec = parameters_to_vector(list(model.parameters()))
            # distance_loss_cos = 1 - torch.dot(wg_hat_vec, model_vec) / (torch.norm(wg_hat_vec) * torch.norm(model_vec))
            distance_loss_eu = torch.norm(wg_hat_vec - model_vec)
            distance_loss = distance_loss_eu

            loss = (1 - alpha) * loss + alpha * distance_loss
        elif pgd_attack and attack_method == "cosine_constrained":
            # distance_loss_cos = model_dist_norm_var(model, target_params_variables, target_model, norm='cos')
            wg_hat_vec = parameters_to_vector(list(wg_hat.parameters()))
            model_vec = parameters_to_vector(list(model.parameters()))
            distance_loss_cos = 1 - torch.dot(wg_hat_vec, model_vec) / (torch.norm(wg_hat_vec) * torch.norm(model_vec))
            # distance_loss_eu = torch.norm(wg_hat_vec - model_vec)
            distance_loss = distance_loss_cos
            loss = (1 - alpha) * loss + alpha * distance_loss
            # print(cosine_alpha)
            ...
            if proj == "l_inf":
                ...
                for i in range(len(w)):
                    ...
                    if True:  # i == 6 or i == 8 or i == 10 or i == 0 or i == 18:
                        ...
                        wf = w1 + w2 + w3
                        w[i].data = wf.data
            elif attack_method in ["cosine_constrained", "constrained_and_scale"]:
                adv_optimizer.step()
            else:
                # do l2_projection
                adv_optimizer.step()
...
class FrequencyFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        if self.attack_method in ["pgd", "cosine_constrained", "constrained_and_scale"]:
        ...
        elif arguments["defense_technique"] == "scope":
            self._defender = Scope(num_workers=self.part_nets_per_round, num_adv=1)
    ...
    def run(self):
        ...
        for flr in range(1, self.fl_round + 1):
                ...
                for net_idx, net in enumerate(net_list):
                    ...
                    if net_idx == 0:
                        ...
                        for e in range(1, self.adversarial_local_training_period + 1):
                            ...
                            if self.defense_technique in ('krum', 'multi-krum', 'multi-metrics', 'scope'):
                                train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e,
                                      ...
                                      prox_attack=self.prox_attack, wg_hat=wg_hat, attack_method=self.attack_method)
                            else:
                                train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e,
                                      log_interval=self.log_interval, criterion=self.criterion,
            ...
            elif self.defense_technique == "scope":
                net_list, net_freq = self._defender.exec(client_models=net_list,
                                                         global_model=self.net_avg,
                                                         num_dps=[
